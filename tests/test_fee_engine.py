...
Verifies:
- Unified 85/8/5/2 fee split (miner/validator/staking/protocol)
...
def test_basic_fee_calculation():
    """2% protocol + 8% validator + 5% staking + 3% subnet = 18%, miner gets 82%."""
    ...
    assert abs(breakdown.protocol_fee - 2.0) < 1e-6, f"Protocol fee: {breakdown.protocol_fee}"
    assert abs(breakdown.validator_reward - 8.0) < 1e-6, f"Validator reward: {breakdown.validator_reward}"
    assert abs(breakdown.staking_pool - 5.0) < 1e-6, f"Staking pool: {breakdown.staking_pool}"
    assert abs(breakdown.subnet_fee - 3.0) < 1e-6, f"Subnet fee: {breakdown.subnet_fee}"
    assert abs(breakdown.miner_reward - 82.0) < 1e-6, f"Miner reward: {breakdown.miner_reward}"
    assert breakdown.protocol_fee_rate == 0.02
    assert breakdown.validator_reward_rate == 0.08
    assert breakdown.staking_pool_rate == 0.05
    assert breakdown.subnet_fee_rate == 0.03


def test_zero_subnet_fee():
    """Subnet fee = 0: 2% protocol + 8% validator + 5% staking, miner gets 85%."""
    engine = FeeEngine(config=ProtocolConfig())
    breakdown = engine.calculate(reward_amount=200.0, subnet_fee_rate=0.0)

    assert abs(breakdown.protocol_fee - 4.0) < 1e-6   # 2% of 200
    assert abs(breakdown.validator_reward - 16.0) < 1e-6  # 8% of 200
    assert abs(breakdown.staking_pool - 10.0) < 1e-6  # 5% of 200
    assert abs(breakdown.subnet_fee - 0.0) < 1e-6
    assert abs(breakdown.miner_reward - 170.0) < 1e-6  # 85% of 200


def test_max_subnet_fee():
    """Max subnet fee = 20%: 2% protocol + 8% validator + 5% staking + 20% subnet."""
    engine = FeeEngine(config=ProtocolConfig())
    breakdown = engine.calculate(reward_amount=100.0, subnet_fee_rate=0.20)

    assert abs(breakdown.protocol_fee - 2.0) < 1e-6
    assert abs(breakdown.validator_reward - 8.0) < 1e-6
    assert abs(breakdown.staking_pool - 5.0) < 1e-6
    assert abs(breakdown.subnet_fee - 20.0) < 1e-6
    assert abs(breakdown.miner_reward - 65.0) < 1e-6
...
def test_priority_multiplier():
    ...
    assert abs(breakdown.protocol_fee - 3.0) < 1e-6  # 2% of 150
    assert abs(breakdown.validator_reward - 12.0) < 1e-6  # 8% of 150
    assert abs(breakdown.staking_pool - 7.5) < 1e-6  # 5% of 150
    assert abs(breakdown.subnet_fee - 4.5) < 1e-6  # 3% of 150
    assert abs(breakdown.miner_reward - 123.0) < 1e-6  # 150 - 3 - 12 - 7.5 - 4.5
...
def test_miner_reward_distribution():
    ...
    rewards = engine.distribute_miner_rewards(85.0, scores)

    assert len(rewards) == 3
    total = sum(rewards.values())
    assert abs(total - 85.0) < 0.01, f"Total rewards: {total}"
...
def test_dynamic_fee_capped():
    ...
    # Protocol fee should be at most 4.0 (2% * 2x cap)
    assert breakdown.protocol_fee <= 4.0 + 1e-6, f"Dynamic fee too high: {breakdown.protocol_fee}"


def test_total_deposit_matches():
    """total_deposit = miner + validator + protocol + subnet + staking."""
    engine = FeeEngine(config=ProtocolConfig())
    breakdown = engine.calculate(reward_amount=100.0, subnet_fee_rate=0.05)
