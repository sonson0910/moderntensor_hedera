...
* **85% Miner Reward:** Goes to the winning AI node.
* **8% Validator Reward:** Split among validators weighted by stake × reputation.
* **5% Staking Pool:** Distributed to passive stakers via StakingVaultV2.
* **2% Protocol Fee:** Goes to the ModernTensor DAO Treasury.

### 6.3 Dual Reward Sources

* **Benchmark Tasks (Emission-funded):** Validators continuously evaluate miners using synthetic tasks. Funded by protocol emission (25M MDT/year, halving every 2 years). No one pays out-of-pocket — similar to Bittensor's incentive loop.
* **Paid Tasks (Marketplace):** External customers submit tasks with MDT payment. Miners compete, validators verify.

### 6.4 Dynamic Calibration

* **Merit-Based Weights:** Miner weight = `performance^2 × reliability`. Updated every epoch by `WeightCalculator`. No stake-based advantage — pure meritocracy.
* **Validator Weights:** `sqrt(stake / min_stake) × reliability`. Stake provides skin-in-the-game.
